...
A TypeScript port of the [GWEN](https://github.com/garrynewman/GWEN) C++ GUI library, rendered with WebGL2. GwenJs brings GWEN's retained-mode control set: buttons, windows, trees, docking panels, colour pickers, property grids to the web as a single file with zero runtime dependencies. Touch and desktop share the same input path; the skin is rasterised procedurally into a GPU atlas at startup.
