...
(() => {
  ...
  var FontAtlas = class {
    ...
    renderText(f, pos, text, renderer2) {
      this.ensureFont(f, renderer2.getFontScale());
      const handle = f.data;
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        renderer2.drawFontGlyphBacking(
          this.texture,
...
void main() {
  ...
  var WebGL2Renderer = class extends Renderer {
    ...
    startClip() {
      ...
      const sy = canvas2.height - Math.round((r.y + r.h) * s);
    ...
    drawFontGlyphBacking(atlasTexture, xBacking, yBacking, wBacking, hBacking, u0, v0, u1, v1) {
      ...
      const dpr2 = this.dpr;
      const cssX = xBacking / dpr2 + this.m_renderOffset.x;
      const cssY = yBacking / dpr2 + this.m_renderOffset.y;
      const tx = Math.round(cssX * dpr2) / dpr2;
      const ty = Math.round(cssY * dpr2) / dpr2;
      const tw = wBacking / dpr2;
      const th = hBacking / dpr2;
      this.writeQuad(tx, ty, tw, th, u0, v0, u1, v1);
    }
    fontAtlasOrThrow() {
      if (!this.fontAtlas) {
    ...
    updateProjection() {
      const { gl, canvas: canvas2 } = this;
      const w = canvas2.clientWidth || canvas2.width || 1;
      const h = canvas2.clientHeight || canvas2.height || 1;
    ...
    buildProgram(vertSrc, fragSrc) {
      ...
      if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
        const log2 = gl.getProgramInfoLog(prog) ?? "(no log)";
        gl.deleteProgram(prog);
        throw new Error(`shader link failed: ${log2}`);
      }
      gl.deleteShader(vs);
      gl.deleteShader(fs);
      return prog;
    ...
    compileShader(type, src) {
      ...
      if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
        const log2 = gl.getShaderInfoLog(sh) ?? "(no log)";
        gl.deleteShader(sh);
        throw new Error(`shader compile failed: ${log2}`);
      }
      return sh;
    }
  };
  ...
  var Base = class _Base {
    ...
    recurseLayout(skin2) {
      if (this._skin) skin2 = this._skin;
      ...
      this.postLayout(skin2);
      const canvas2 = this.getCanvas();
      if (canvas2 && this._tabable && !this._disabled) {
        if (!canvas2.firstTab) canvas2.firstTab = this;
        if (!canvas2.nextTab) canvas2.nextTab = this;
        canvas2.tabList.push(this);
      }
      if (canvas2 && canvas2.keyboardFocus === this) {
        canvas2.nextTab = null;
      }
    }
    ...
    doRender(skin2) {
      if (this._skin) skin2 = this._skin;
      this.think();
      this.renderRecursive(skin2, this._bounds);
    }
    renderRecursive(skin2, cliprect) {
      const render = skin2.renderer;
      const oldOffset = render.getRenderOffset();
      render.addRenderOffset(cliprect);
      this.renderUnder(skin2);
      ...
      render.setClipRegion(oldClip);
      render.startClip();
      this.renderOver(skin2);
      this.renderFocus(skin2);
      render.endClip();
      render.setRenderOffset(oldOffset);
    }
    ...
    renderFocus(skin2) {
      ...
      skin2.drawKeyboardHighlight(this, this._renderBounds, 3);
    }
    // =======================================================================
    // Skin
    // =======================================================================
    ...
    focus() {
      const canvas2 = this.getCanvas();
      if (!canvas2) return;
      if (canvas2.keyboardFocus === this) return;
      if (canvas2.keyboardFocus) canvas2.keyboardFocus.onLostKeyboardFocus();
      canvas2.keyboardFocus = this;
    ...
    onKeyTab(down) {
      if (!down) return true;
      const canvas2 = this.getCanvas();
      if (!canvas2) return true;
      const fullList = canvas2.tabList;
      if (fullList.length === 0) return true;
      const reverse = canvas2.isShiftDown();
      const focus = canvas2.keyboardFocus;
    ...
    updateCursor() {
      const canvas2 = this.getCanvas();
      if (canvas2) canvas2.setCursor(this._cursor);
    }
    ...
    dispose() {
      ...
      const canvas2 = this.getCanvas();
      if (canvas2) {
        if (canvas2.hoveredControl === this) canvas2.hoveredControl = null;
        if (canvas2.keyboardFocus === this) canvas2.keyboardFocus = null;
        if (canvas2.mouseFocus === this) canvas2.mouseFocus = null;
      }
    }
    // =======================================================================
  ...
  var Skin = class {
    constructor(renderer2) {
      ...
      this.renderer = renderer2;
      this.dynamicSkin = new DynamicSkin(renderer2);
    }
    init() {
  ...
  var Dragger = class extends Base {
    ...
    onMouseClickLeft(x, y, pressed) {
      ...
      if (pressed) {
        const canvas3 = this.getCanvas();
        if (canvas3) canvas3.mouseFocus = this;
      ...
      this._depressed = false;
      const canvas2 = this.getCanvas();
      if (canvas2) canvas2.mouseFocus = null;
  ...
  var ScrollBarBar = class extends Dragger {
    ...
    render(skin2) {
      skin2.drawScrollBarBar(this, this._depressed, this.isHovered(), this._horizontal);
    }
  };
  ...
  var Text = class _Text extends Base {
    ...
    refreshSize() {
      ...
      if (this._text.length > 0) {
        const size = skin2.renderer.measureText(font2, this._text);
        w = size.x;
        h = size.y;
      } else {
        w = 1;
    ...
    refreshSizeWrap() {
      ...
      const lineH = Math.ceil(skin2.renderer.measureText(font2, "Ag").y);
      const lines = [];
      ...
      for (let segIdx = 0; segIdx < hardSegments.length; segIdx++) {
        ...
        for (const tok of tokens) {
          ...
          const w = skin2.renderer.measureText(font2, tok).x;
          if (isSpace) {
    ...
    getCharacterPosition(i) {
      ...
      const prefixSize = prefix.length > 0 ? skin2.renderer.measureText(font2, prefix) : point(0, font2.size);
      const charW = tail.length > 0 ? skin2.renderer.measureText(font2, tail).x : 0;
      return rect(
  ...
  var Label = class _Label extends Base {
    ...
    dispose() {
      if (this._createdFont) {
        const skin2 = this.getSkinOrNull();
        if (skin2) skin2.releaseFont(this._createdFont);
        this._createdFont = null;
      }
      super.dispose();
    }
  ...
  var ImagePanel = class extends Base {
     ...
     */
    setTextureFromSource(source, renderer2) {
      const t = texture();
      renderer2.loadTextureFromSource(t, source);
      this._texture = t;
      this.redraw();
    }
    getTexture() {
     ...
     */
    static loadFromURL(url, renderer2) {
      return new Promise((resolve, reject) => {
        const img = new Image();
        img.crossOrigin = "anonymous";
        img.onload = () => {
          const t = texture(url);
          renderer2.loadTextureFromSource(t, img);
          resolve(t);
        };
        img.onerror = () => {
          const t = texture(url);
  ...
  var Button = class extends Label {
    ...
    render(skin2) {
      ...
      skin2.drawButton(this, drawDepressed, hovered, this.isDisabled());
    }
    // =====================================================================
    // Mouse
    // =====================================================================
  ...
  var ScrollBarButton = class extends Button {
    ...
    render(skin2) {
      skin2.drawScrollButton(this, this._direction, this.isDepressed(), this.isHovered(), this.isDisabled());
    }
  };
  ...
  var HorizontalScrollBar = class extends BaseScrollBar {
    ...
    onMouseClickLeft(x, y, pressed) {
      if (!pressed) {
        this._depressed = false;
        const canvas3 = this.getCanvas();
        if (canvas3 && canvas3.mouseFocus === this) canvas3.mouseFocus = null;
        return;
      }
      this._depressed = true;
      const canvas2 = this.getCanvas();
    ...
    render(skin2) {
      skin2.drawScrollBar(this, true, this._depressed);
    }
  };
  var VerticalScrollBar = class extends BaseScrollBar {
    ...
    render(skin2) {
      skin2.drawScrollBar(this, false, this._depressed);
    }
  };
  ...
  var MenuItem = class _MenuItem extends Button {
    ...
    getMenu() {
      if (!this._menu) {
        const canvas2 = this.getCanvas();
        this._menu = new Menu(canvas2 ?? null);
        this._menu.hide();
        if (!this._onStrip) {
  ...
  var Menu = class extends ScrollControl {
    ...
    open(pos) {
      ...
      this.bringToFront();
      const canvas2 = this.getCanvas();
      const p = pos ?? canvas2?.mousePosition ?? point(0, 0);
      this.setPos(p.x, p.y);
    }
    close() {
      this.hide();
      ...
      if (this._deleteOnClose) {
        const canvas2 = this.getCanvas();
        if (canvas2 && typeof canvas2.addDelayedDelete === "function") {
          canvas2.addDelayedDelete(this);
        }
      }
    }
    ...
    // =====================================================================
    render(skin2) {
      skin2.drawMenu(this, this._disableIconMargin);
    }
    renderUnder(skin2) {
      super.renderUnder(skin2);
      skin2.drawShadow(this);
    }
    // =====================================================================
    ...
    layout(skin2) {
      ...
      if (inner) {
        ...
        const canvas2 = this.getCanvas();
        if (canvas2 && this.y() + h > canvas2.height()) {
          h = canvas2.height() - this.y();
        }
        const pad = this.getPadding();
  ...
  var Canvas = class extends Base {
    constructor(skin2, htmlCanvas2) {
      ...
      this.skin = skin2;
      this.renderer = skin2.renderer;
      this.htmlCanvas = htmlCanvas2;
      this.setSkin(skin2);
      ...
      this._detachInput = attachInput(htmlCanvas2, this);
    }
    // ======================================================================
    // Canvas identity
    // ======================================================================
    ...
    renderToolTip() {
      ...
      tip.setHidden(false);
      const renderer2 = this.skin.renderer;
      const savedOffset = renderer2.getRenderOffset();
      renderer2.setRenderOffset(point(savedOffset.x + tx, savedOffset.y + ty));
      this.skin.drawToolTip(tip);
      renderer2.setRenderOffset(savedOffset);
    ...
    renderDragPreview() {
      ...
      const h = dc.height();
      if (w <= 0 || h <= 0) return;
      const oldOffset = renderer2.getRenderOffset();
      const targetX = this.mousePosition.x - ho.x;
      const targetY = this.mousePosition.y - ho.y;
      renderer2.setRenderOffset(point(targetX - dc.x(), targetY - dc.y()));
      dc.doRender(this.skin);
      renderer2.setRenderOffset(oldOffset);
    }
    // ======================================================================
    // Hover
    // ======================================================================
  ...
  var Rectangle = class extends Base {
    ...
    render(skin2) {
      skin2.renderer.setDrawColor(this._color);
      skin2.renderer.drawFilledRect(this.getRenderBounds());
    }
  };
  ...
  var ProgressBar = class extends Label {
    ...
    // =====================================================================
    render(skin2) {
      skin2.drawProgressBar(this, this._horizontal, this._progress);
    }
  };
  ...
  var GroupBox = class extends Label {
    ...
    // =====================================================================
    render(skin2) {
      skin2.drawGroupBox(this, this.textX(), this.textHeight(), this.textWidth());
    }
  };
  ...
  var CheckBox = class extends Button {
    ...
    // =====================================================================
    render(skin2) {
      skin2.drawCheckBox(this, this._checked, this.isDepressed());
    }
    info() {
      const i = eventInfo();
    ...
    rebuild(skin2) {
      ...
      for (let bi = 0; bi < this._blocks.length; bi++) {
        ...
        for (let ti = 0; ti < tokens.length; ti++) {
          ...
          if (font2) {
            const size = skin2.renderer.measureText(font2, tok);
  ...
  var SliderBar = class extends Dragger {
    ...
    render(skin2) {
      skin2.drawSlideButton(this, this._depressed, this._horizontal);
    }
  };
  ...
  var _HorizontalSlider = class _HorizontalSlider extends Slider {
    ...
    renderUnder(skin2) {
      ...
      skin2.drawKeyboardHighlight(this, rect(-3, boxY, this.width() + 6, boxH), 0);
    }
  ...
  var _VerticalSlider = class _VerticalSlider extends Slider {
    ...
    renderUnder(skin2) {
      ...
      skin2.drawKeyboardHighlight(this, rect(boxX, -3, boxW, this.height() + 6), 0);
    }
    renderFocus(_skin) {
    }
  };
  ...
  var RadioButton = class extends CheckBox {
    ...
    render(skin2) {
      skin2.drawRadioButton(this, this.isChecked(), this.isDepressed());
    }
  };
  var LabeledRadioButton = class extends Base {
    ...
    renderFocus(skin2) {
      ...
      if (!canvas2 || canvas2.keyboardFocus !== this) return;
      if (!this.isTabable()) return;
      skin2.drawKeyboardHighlight(this, this.getRenderBounds(), 0);
    }
  };
  ...
  var TextBox = class extends Label {
    ...
    makeCaretVisible() {
      ...
      const skin2 = this.getSkin();
      const font2 = this.getFont() ?? skin2.getDefaultFont();
      const pad = this.getPadding();
      const visibleW = this.width() - pad.left - pad.right;
      if (visibleW <= 0) return;
      const caretX = skin2.renderer.measureText(font2, this.displayedText().substring(0, this._cursorPos)).x;
    ...
    charIndexAt(localX, localY) {
      ...
      if (this._text.getWrap() && localY !== void 0) {
        const lineH = Math.max(1, Math.ceil(skin2.renderer.measureText(font2, "Ag").y));
      ...
      for (let i = 1; i <= lineText.length; i++) {
        const w = skin2.renderer.measureText(font2, lineText.substring(0, i)).x;
        const px = pad.left + w;
    ...
    onMouseMoved(x, y, dx, dy) {
      const canvas2 = this.getCanvas();
      if (!canvas2 || canvas2.mouseFocus !== this) return;
      const local = this.canvasPosToLocal(point(x, y));
      const idx = this.charIndexAt(local.x, local.y);
      this.setCursorPos(idx);
    }
    ...
    renderOver(skin2) {
      ...
      if (this.hasSelection()) {
        const a = Math.min(this._cursorPos, this._cursorEnd);
        const b = Math.max(this._cursorPos, this._cursorEnd);
        const xA = skin2.renderer.measureText(font2, text.substring(0, a)).x;
        const xB = skin2.renderer.measureText(font2, text.substring(0, b)).x;
        const selRect = rect(
          pad.left + xA - off,
          pad.top,
          xB - xA,
        ...
        skin2.renderer.setDrawColor(color(50, 170, 255, 200));
        skin2.renderer.drawFilledRect(selRect);
      }
      if (this._caretVisible) {
        const caretX = pad.left + skin2.renderer.measureText(font2, text.substring(0, this._cursorPos)).x - off;
    ...
    isShiftDown() {
      const canvas2 = this.getCanvas();
      return canvas2 !== null && typeof canvas2.isShiftDown === "function" && canvas2.isShiftDown();
    }
  };
  ...
  var TextBoxMultiline = class extends TextBox {
    ...
    renderOver(skin2) {
      ...
      if (cursorPos !== cursorEnd) {
        ...
        const la = locate(a);
        const lb = locate(b);
        const xA = skin2.renderer.measureText(font2, la.prefix).x;
        const xB = skin2.renderer.measureText(font2, lb.prefix).x;
        skin2.renderer.setDrawColor(color(50, 170, 255, 200));
        if (la.line === lb.line) {
          skin2.renderer.drawFilledRect(rect(pad.left + xA, pad.top + la.line * lineH, xB - xA, lineH));
        } else {
          const fullW = this.width() - pad.left - pad.right;
          skin2.renderer.drawFilledRect(rect(pad.left + xA, pad.top + la.line * lineH, fullW - xA, lineH));
          for (let l = la.line + 1; l < lb.line; l++) {
            skin2.renderer.drawFilledRect(rect(pad.left, pad.top + l * lineH, fullW, lineH));
          }
          skin2.renderer.drawFilledRect(rect(pad.left, pad.top + lb.line * lineH, xB, lineH));
        }
      }
      if (this._caretVisible) {
    ...
    isShiftHeld() {
      ...
      const fn = canvas2.isShiftDown;
      return typeof fn === "function" ? fn.call(canvas2) : false;
    }
  };
  ...
  var NumericUpDownButton_Up = class extends Button {
    ...
    render(skin2) {
      skin2.drawNumericUpDownButton(this, this.isDepressed(), true);
    }
  };
  var NumericUpDownButton_Down = class extends Button {
    ...
    render(skin2) {
      skin2.drawNumericUpDownButton(this, this.isDepressed(), false);
    }
  };
  ...
  var ToggleButton = class extends Button {
    ...
    render(skin2) {
      skin2.drawTreeButton(this, this._open);
    }
  ...
  var TreeNodeText = class extends Button {
    ...
    render(skin2) {
      if (this.isDisabled()) {
        this.setTextColor(skin2.colors.button.disabled);
      } else if (this.isDepressed() || this.getToggleState()) {
        this.setTextColor(skin2.colors.tree.selected);
      } else if (this.isHovered()) {
        this.setTextColor(skin2.colors.tree.hover);
      } else {
        this.setTextColor(skin2.colors.tree.normal);
      }
      super.render(skin2);
    }
  };
  ...
  var ActionBarSeparator = class extends Base {
    ...
    render(skin2) {
      ...
      skin2.renderer.setDrawColor(color(140, 140, 140, 255));
      if (r.h > r.w) {
        const cx = r.x + Math.floor(r.w / 2);
        skin2.renderer.drawFilledRect(rect(cx, r.y + 4, 1, Math.max(0, r.h - 8)));
      } else {
        const cy = r.y + Math.floor(r.h / 2);
        skin2.renderer.drawFilledRect(rect(r.x + 4, cy, Math.max(0, r.w - 8), 1));
      }
    }
  };
  var ActionBar = class extends Base {
    ...
    setActiveButton(btn) {
      if (this._sectionMode) {
        ...
        const prev2 = this._sections[idx].active;
        if (prev2 === btn) return;
        this._sections[idx].active = btn;
        btn.setToggleState(true);
        if (prev2 && prev2 !== btn) prev2.setToggleState(false);
        return;
      }
    ...
    attachRadioHandlers(btn) {
      btn.onToggleOn.on(() => {
        if (this._sectionMode) {
          ...
          if (prev2 === btn) return;
          this._sections[idx].active = btn;
          if (prev2) prev2.setToggleState(false);
          return;
        }
  ...
  var CollapsibleCategory = class extends Base {
    ...
    render(skin2) {
      skin2.drawCategoryInner(this, this._headerButton.getToggleState());
    }
  };
  ...
  var TabButton = class extends Button {
    ...
    render(skin2) {
      skin2.drawTabButton(this, this.isActive(), this._tabDock);
    }
  ...
  var TabControl = class extends Base {
    ...
    // =====================================================================
    setTabStripPosition(dock2) {
      this._tabStrip.dock(dock2);
      for (const c of this._tabStrip.children) {
        if (c instanceof TabButton) c.setTabDock(dock2);
      }
    }
    setAllowReorder(b) {
    ...
    renderFocus(skin2) {
      ...
      skin2.drawKeyboardHighlight(this, rect(x, y, btn.width(), btn.height()), 0);
    }
    onTabPressed(btn) {
  ...
  var ListBoxRow = class extends Button {
    ...
    onMouseClickLeft(x, y, pressed) {
      ...
      if (!wasDepressed) {
        const canvas2 = this.getCanvas();
        const shift = canvas2 !== null && typeof canvas2.isShiftDown === "function" && canvas2.isShiftDown();
        this._listBox.onRowClicked(this, shift);
      }
    }
    render(skin2) {
      skin2.drawListBoxLine(this, this._selected, this._even);
    }
  };
  ...
  var TreeControl = class extends TreeNode {
    ...
    onNodeSelected(_info) {
      if (this._allowMultiSelect) {
        const canvas2 = this.getCanvas();
        const ctrlHeld = canvas2 !== null && typeof canvas2.isControlDown === "function" && canvas2.isControlDown();
        if (ctrlHeld) return;
      }
      this.deselectAll();
    }
    ...
    // =====================================================================
    render(skin2) {
      if (this.shouldDrawBackground()) skin2.drawTreeControl(this);
    }
  };
  ...
  var ComboBox = class extends Button {
    constructor(parent) {
      ...
      this.setTextPadding(margin(3, 0, 3, 0));
      const canvas2 = this.getCanvas();
      this._menu = new Menu(canvas2 ?? this);
    ...
    openList() {
      const canvas2 = this.getCanvas();
      if (canvas2) this._menu.setParent(canvas2);
    ...
    // =====================================================================
    render(skin2) {
      skin2.drawComboBox(this, this.isDepressed(), this.isMenuOpen());
    }
  };
  ...
  var WindowCloseButton = class extends Button {
    ...
    render(skin2) {
      ...
      skin2.drawWindowCloseButton(this, this.isDepressed(), this.isHovered(), this.isDisabled());
    }
  };
  var WindowMaximizeButton = class extends WindowCloseButton {
    ...
    render(skin2) {
      ...
      skin2.drawWindowMaximizeButton(this, this.isDepressed(), this.isHovered(), this.isDisabled(), this._maximized);
    }
  };
  var WindowMinimizeButton = class extends WindowCloseButton {
    ...
    render(skin2) {
      if (!this.getWindow()) return;
      skin2.drawWindowMinimizeButton(this, this.isDepressed(), this.isHovered(), this.isDisabled());
    }
  };
  ...
  var WindowControl = class _WindowControl extends ResizableControl {
    ...
    destroyModal() {
      if (!this._modal) return;
      const canvas2 = this.getCanvas();
      this.setParent(this._preModalParent ?? canvas2);
      this._modal.dispose();
      this._modal = null;
      this._preModalParent = null;
    }
    ...
    render(skin2) {
      ...
      skin2.drawWindow(this, this._titleBar.bottom(), hasFocus);
    }
    renderUnder(skin2) {
      skin2.drawShadow(this);
    }
  ...
  var DockBase = class _DockBase extends Base {
    ...
    setFieldValue(pos, dock2) {
      if (pos === Pos.Left) this._left = dock2;
      else if (pos === Pos.Right) this._right = dock2;
      else if (pos === Pos.Top) this._top = dock2;
      else if (pos === Pos.Bottom) this._bottom = dock2;
    }
  ...
  var ColorDisplay = class extends Base {
    ...
    // =======================================================================
    render(skin2) {
      skin2.drawColorDisplay(this, this._color);
    }
  };
  ...
  var ColorLerpBox = class extends Base {
    ...
    onMouseClickLeft(x, y, pressed) {
      ...
      const canvas2 = this.getCanvas();
      if (canvas2) canvas2.mouseFocus = pressed ? this : null;
    ...
    render(skin2) {
      if (this._textureDirty) {
        this.bakeTexture(skin2.renderer);
        this._textureDirty = false;
      }
      skin2.renderer.setDrawColor(color(255, 255, 255, 255));
      skin2.renderer.drawTexturedRect(this._texture, this.getRenderBounds());
      skin2.renderer.setDrawColor(color(0, 0, 0, 255));
      skin2.renderer.drawLinedRect(this.getRenderBounds());
      const cur = this.getSelectedColor();
      const avg = (cur.r + cur.g + cur.b) / 3;
      skin2.renderer.setDrawColor(avg < 170 ? color(255, 255, 255, 255) : color(0, 0, 0, 255));
      skin2.renderer.drawLinedRect(rect(this._cursorPos.x - 3, this._cursorPos.y - 3, 6, 6));
    }
    ...
    dispose() {
      const canvas2 = this.getCanvas();
      const renderer2 = canvas2?.renderer;
      if (renderer2 && this._texture.data) renderer2.freeTexture(this._texture);
      super.dispose();
    }
    ...
    bakeTexture(renderer2) {
      ...
      renderer2.loadTextureFromSource(this._texture, off);
    }
    // =======================================================================
    // Internals
    // =======================================================================
  ...
  var ColorSlider = class extends Base {
    ...
    render(skin2) {
      ...
      const h = this.height();
      for (let y = 0; y < h; y += 1) {
        renderer2.setDrawColor(this.getColorAtHeight(y));
        renderer2.drawFilledRect(rect(5, y, w - 10, 1));
      }
      const drawY = this._selectedDist - 3;
      renderer2.setDrawColor(color(0, 0, 0, 255));
      renderer2.drawFilledRect(rect(0, drawY, 5, 5));
      renderer2.drawFilledRect(rect(w - 5, drawY, 5, 5));
      renderer2.drawFilledRect(rect(0, drawY + 2, w, 1));
    }
  ...
  function query(canvas2, text, onYes, onNo, onCancel) {
    const win = new WindowControl(canvas2, "Confirm");
    win.setSize(320, 140);
  ...
  var PropertyRow = class extends Base {
    ...
    render(skin2) {
      ...
      const splitX = this.getParentSplitWidth();
      skin2.drawPropertyRow(this, splitX !== null ? splitX : this._label.width(), editing, hovered);
    }
  ...
  var PropertyTreeNode = class extends TreeNode {
    ...
    render(skin2) {
      ...
      skin2.drawPropertyTreeNode(this, inner.x(), inner.y());
    }
  };
  ...
  // demo/main.ts
  var htmlCanvas = document.getElementById("gwen-canvas");
  ...
  sizeCanvas();
  var renderer = new WebGL2Renderer(htmlCanvas, { devicePixelRatio: dpr });
  renderer.init();
  var skin = new Skin(renderer);
  skin.init();
  ...
  function makeSmileyTexture() {
    ...
    g.stroke();
    const tex = texture("smiley");
    renderer.loadTextureFromSource(tex, c);
    return tex;
  }
  function makeGwenLogoTexture() {
    const c = document.createElement("canvas");
    c.width = 96;
    c.height = 96;
    const g = c.getContext("2d");
    ...
    g.fill();
    const tex = texture("gwen-logo");
    renderer.loadTextureFromSource(tex, c);
    return tex;
  }
  var smileyTexture = makeSmileyTexture();
  var gwenLogoTexture = makeGwenLogoTexture();
  var menuStrip = new MenuStrip(canvas);
  {
  ...
  var globalMenu = new Menu(canvas);
  {
  ...
  var dock = new DockBase(canvas);
  dock.dock(Pos.Fill);
  var leftDock = dock.getLeft();
  leftDock.setWidth(190);
  var sidebarPage = new Base(null);
  var catList = new CollapsibleList(sidebarPage);
  catList.dock(Pos.Fill);
  leftDock.getTabControl()?.addPage("CollapsibleList", sidebarPage);
  var bottomDock = dock.getBottom();
  bottomDock.setHeight(150);
  var outputPage = new Base(null);
  var outputList = new ListBox(outputPage);
  outputList.dock(Pos.Fill);
  ...
  var centerPanel = new Base(dock);
  centerPanel.dock(Pos.Fill);
  centerPanel.setMargin(margin(4, 4, 4, 4));
  var demoSections = {};
  var demoRows = {};
  function addDemo(category, name, build) {
    const panel = new Base(centerPanel);
    panel.dock(Pos.Fill);
    panel.hide();
    build(panel);
  ...
  addDemo(basicCat, "Label", (p) => {
    ...
    const clickable = new LabelClickable(p);
    ...
    const alignH = [
      ...
      [Pos.Top | Pos.CenterH, "Top-Center"],
      [Pos.Top | Pos.Right, "Top-Right"],
      [Pos.CenterV | Pos.Left, "Center-Left"],
      [Pos.Center, "Center"],
      [Pos.CenterV | Pos.Right, "Center-Right"],
      [Pos.Bottom | Pos.Left, "Bottom-Left"],
      [Pos.Bottom | Pos.CenterH, "Bottom-Center"],
  ...
  addDemo(nonInteractiveCat, "ProgressBar", (p) => {
    const v1 = new ProgressBar(p);
    v1.setBounds(150, 20, 30, 200);
    v1.setVertical();
    v1.setProgress(0.25);
    const v2 = new ProgressBar(p);
    v2.setBounds(190, 20, 30, 200);
    v2.setVertical();
    v2.setProgress(0.4);
    const v3 = new ProgressBar(p);
  ...
  addDemo(controlsCat, "MenuStrip", (p) => {
    ...
    info.setBounds(20, 20, 500, 20);
    info.setAlignment(Pos.Left | Pos.CenterV);
    const more = new Label(p);
    more.setText("Click any of File / Edit / Help to see it in action.");
    more.setBounds(20, 45, 500, 20);
  });
  var containersCat = catList.add("Containers");
  addDemo(containersCat, "Window", (p) => {
    class CenteredColumn extends Base {
      postLayout(skin2) {
        super.postLayout(skin2);
        const w = this.width();
        for (const c of this.children) {
  ...
  addDemo(containersCat, "Properties", (p) => {
    ...
    props.add("Last Name");
    const tree = new PropertyTree(p);
    tree.setBounds(200, 10, 260, 320);
    const widenLabels = (g) => g.getSplitter().setPos(110, 0);
    {
    ...
    }
    {
      ...
      widenLabels(item);
      item.add("More Items");
      item.addRow("Checkbox", new PropertyCheckbox(item), "1");
      item.add("To Fill");
      item.addRow("ColorSelector", new PropertyColorSelector(item), "255 0 0");
      item.add("Out Here");
      const numeric = new PropertyNumeric(item);
      numeric.getNumericUpDown().setMin(0);
      numeric.getNumericUpDown().setMax(100);
      item.addRow("Stepper", numeric, "42");
      const combo = new PropertyComboBox(item);
      ...
      const fileProp = new PropertyFile(item);
  ...
  addDemo(containersCat, "ActionBar", (p) => {
    ...
    quick.addSeparator();
    const fontCombo = new ComboBox(quick);
    ...
    const sectioned = new ActionBar(p);
  ...
  addDemo(containersCat, "PageControl", (p) => {
    ...
    }
    {
      ...
      if (page) {
        ...
        l.setBounds(20, 20, 300, 20);
        const cb = new ComboBox(page);
        cb.setBounds(20, 50, 200, 22);
        cb.addItem("Red");
        cb.addItem("Green");
        cb.addItem("Blue");
        const chk = new CheckBoxWithLabel(page);
  ...
  addDemo(nonStandardCat, "RightClick", (p) => {
    const panelMenu = new Menu(canvas);
    ...
    deferred.onContextMenuRequest = () => null;
    const note = new Label(p);
  ...
  addDemo(nonStandardCat, "ColorPicker", (p) => {
    const cp = new ColorPicker(p);
    cp.setBounds(20, 20, 256, 150);
    cp.setColor(color(255, 100, 50, 255));
    const hsv2 = new HSVColorPicker(p);
    hsv2.setBounds(300, 20, 256, 180);
  });
  addDemo(nonStandardCat, "FilePicker", (p) => {
    const intro = new Label(p);
    ...
    const grid = new Properties(p);
    grid.setBounds(20, 130, 500, 60);
    const propRow = grid.addRow("Attachment", new PropertyFile(grid));
    propRow.onChange.on(() => {
      const prop = propRow.getProperty();
      const f = prop ? prop.getFile() : null;
      log(`PropertyFile: ${f ? f.name : "(cleared)"}`);
    });
  });
  showDemo("Button");
  var demoWindow = window;
  demoWindow.Gwen = src_exports;
  ...
  console.log(
    `GwenJs v${VERSION} demo boot \u2014 ${Object.keys(demoSections).length} demos, ${skin.dynamicSkin.regions.size} skin regions.`
  );
})();
